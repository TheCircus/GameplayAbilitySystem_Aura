                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair  : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		if (UAuraAbilitySystemLibrary::IsRadialDamage(EffectContextHandle))
		{
			...
			// 2. create delegate OnDamageDelegate, broadcast damaged received in TakeDamage *DONE*
			// 3. Bind lambda to OnDamageDelegate on the Victim here. *DONE*
			// 4. Call UGameplayStatics::ApplyRadialDamageWithFalloff to cause damage (this will result in TakeDamage being
			//		called on the Victim, which will then broadcast OnDamageDelegate) *DONE*
